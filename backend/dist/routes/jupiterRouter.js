...
const jupiterRouter = express_1.default.Router();
jupiterRouter.use(express_1.default.json());
...
jupiterRouter.get("/order", async (req, res) => {
    try {
        const { inputMint, outputMint, amount, taker, slippageBps, referralAccount, referralFee, } = req.query;
        ...
        if (referralFee)
            url.searchParams.set("referralFee", String(referralFee));
        const headers = {
...
async function loadAllTokens() {
    ...
    if (!r.ok)
        throw new Error(`Jupiter token fetch failed: HTTP ${r.status}`);
...
jupiterRouter.get("/tokens/basic", async (req, res) => {
    try {
        const symbolsParam = req.query.symbols ?? "SOL,USDT";
...
jupiterRouter.get("/tokens/search", async (req, res) => {
    try {
        ...
        const filtered = allTokens.filter((t) => t.name.toLowerCase().includes(queryLower) ||
